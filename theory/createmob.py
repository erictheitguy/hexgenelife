...
from theory.db_connection import get_db_connection, insert_mob
...
INHERITANCE_MODES = {
// ...existing code...
    "recessive": "Traits only expressed if both parents carry them (simplified: average)"
}
# Inheritance modes with descriptions
INHERITANCE_MODES_DESC = {
// ...existing code...
...
# Database connection and collection setup removed. Connection will be established per operation.
...
for i in range(1, number_to_create):
    ...
    total_hexes = theory.db_connection.count_all_hex_tiles()
    random_tile_number = random.randint(0, (total_hexes - 1))
    # random_tile = hex_tiles_collection.find().skip(random_tile_number).limit(-1)
    random_tile = theory.db_connection.get_random_hex_tile(random_tile_number)
