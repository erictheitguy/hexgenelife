...
while i < 100:
    ...
    # get nearest 7 tiles
    hex_search = hex_tiles_collection.database.command({"geoNear": "hex_tiles", "near": [life_loc_x,life_loc_y], "num": 7})

    # hex_search = hex_tiles_collection.find( {"loc": {"$geoIntersects" : {"$geometry" : { "type": "Point" , "coordinates"
    # :[ life_loc_x, life_loc_y] } } } } )

    # lets create some emtpy hexagon poly in this loop

    create_new_poly = False
    point_inside = False
    if hex_search["stats"]["objectsLoaded"] > 0:
        print("we found some hexagons",hex_search["stats"]["objectsLoaded"])
        hex_count = hex_search["stats"]["objectsLoaded"]

        for hexagon_tile_found in hex_search["results"]:
            ...
            CX1,CY1 = hexagon_tile_found["obj"]["centerXY"]

            # determine if we are inside of it ray tracing method
            hexagon_poly = hexagon_tile_found["obj"]["loc"]["coordinates"]
            ...
            if point_inside == True:
                ...
                old_hex_center = hexagon_tile_found["obj"]["centerXY"]
                # do nothing else
                create_new_poly = False
                break
            else:
